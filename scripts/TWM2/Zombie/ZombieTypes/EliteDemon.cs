...
function EDZombiemovetotarget(%zombie){
   if(!isobject(%zombie))
	return;
   ...
   if(%closestDistance <= $zombie::detectDist){
	if(%zombie.hastarget != 1 && %closestdistance >= 10 && %closestdistance <= 150){
	   EDzombieFire(%zombie,%closestclient);
	   %zombie.canjump = 0;
   ...
   %zombie.moveloop = schedule(500, %zombie, "EDZombiemovetotarget", %zombie);
}

function EDZombieFire(%zombie,%closestclient){
